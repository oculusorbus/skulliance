 ...
 <style>
        ...
        */
        ...
        #try-again, #leaderboard {
            ...
            width: 250px; /* Maintained width from previous request */
            box-sizing: border-box;
            text-align: center;
        }
...
<body style="background: #0F0F0F; margin: 0; height: 100vh; display: flex; justify-content: center; align-items: center; overflow: hidden;">
    ...
    <!-- Hidden element for row0 visibility toggle (if needed elsewhere in your project) -->
    <div id="row0" style="visibility: hidden;"></div>
...
class Match3Game {
    ...
    async endGame() {
        ...
        // Detonate only initial bombs on the board
        let bombPositions = this.getAllBombPositions();
        console.log(`Found ${bombPositions.length} initial bombs for grand finale`);
        ...
        while (bombPositions.length > 0) {
            ...
            this.renderBoard();
            await new Promise(resolve => setTimeout(resolve, 500)); // Maintained for stability and speed

            // Check for new matches but don’t detonate new bombs automatically
            let hasMatches = await this.resolveAllMatches();
            while (hasMatches) {
                console.log('New matches found after bomb detonation, resolving...');
                await this.cascadeTiles();
                this.showerTiles();
                this.renderBoard();
                await new Promise(resolve => setTimeout(resolve, 500)); // Maintained for stability and speed
                hasMatches = await this.resolveAllMatches();
            }

            // Don’t add newly created bombs to the chain—only detonate initial bombs
            console.log(`Detonated at (${bomb.x}, ${bomb.y}), no new bombs added to chain`);
        }

        // Clear all remaining matches with extended checks, but don’t detonate new bombs
        console.log('Clearing all remaining matches...');
        let hasMatches = true;
        let iterations = 0;
        const maxIterations = 100; // Maintained for thoroughness
        while (hasMatches && iterations < maxIterations) {
            hasMatches = await this.resolveAllMatches(); // Use thorough match check
            console.log(`Match check iteration ${iterations + 1}, hasMatches: ${hasMatches}`);
            if (hasMatches) {
                await this.cascadeTiles();
                this.showerTiles();
                this.renderBoard();
                await new Promise(resolve => setTimeout(resolve, 700)); // Maintained for stability
        ...
        while (moved && iterations < maxIterations) {
            ...
            if (moved) {
                ...
                // Check for any final matches
                hasMatches = await this.resolveAllMatches();
                while (hasMatches) {
                    console.log('Final match found, resolving...');
                    await this.cascadeTiles();
                    this.showerTiles();
                    this.renderBoard();
                    await new Promise(resolve => setTimeout(resolve, 700)); // Maintained for stability
    ...
    // Enhanced match resolution method to check the entire board thoroughly
        ...
        for (let y = 0; y < this.height; y++) {
            for (let x = 0; x < this.width; x++) {
                ...
                if (matchResult.hasMatches && matchResult.matches.size >= 3) {
                    console.log(`Found match at (${x}, ${y}) with size ${matchResult.matches.size}`);
                    const firstTile = this.board[y][x];
                    if (firstTile.special) {
    ...
    async cascadeTiles() {
        ...
        while (hasMatches) {
            ...
            hasMatches = await this.resolveMatches(); // Ensure bombs only detonate via matches
        }
    }
    ...
    async handleBombMatches(matches, bombType, bombX, bombY) {
        ...
        // Capture the board state before any changes to ensure all bombs are tracked
        const initialBombs = this.getAllBombPositions();
        console.log(`Initial bombs before detonation:`, initialBombs);
        ...
        if (bombType === 'carbon') {
            ...
            console.log(`Carbon bomb detonated at (${bombX}, ${bombY}) during match, +${this.bonusScores.carbonDetonation} bonus`);
            const newBombs = await this.clearRowAndColumn(bombX, bombY);
            // Chain detonate all existing bombs in the same row and column, including diamond bombs
            await this.chainDetonateBombs(initialBombs, bombX, bombY, 'carbon');
        } else if (bombType === 'diamond') {
            this.score += this.bonusScores.diamondDetonation;
            console.log(`Diamond bomb detonated at (${bombX}, ${bombY}) during match, +${this.bonusScores.diamondDetonation} bonus`);
            const newBombs = await this.clearBoard(bombX, bombY);
            // Chain detonate all existing bombs on the board, including carbon bombs, without existence checks that might skip bombs
            await this.chainDetonateBombs(initialBombs, null, null, 'diamond');
        }
    ...
    async chainDetonateBombs(initialBombs, bombX, bombY, bombType) {
        let bombsToDetonate = [];
        if (bombType === 'carbon') {
            // Detonate all existing bombs in the same row and column, including both carbon and diamond bombs, without exclusions
            for (const bomb of initialBombs) {
                if (bomb.x === bombX || bomb.y === bombY) {
                    bombsToDetonate.push(bomb);
                }
            }
            console.log(`Carbon bomb chain reaction at (${bombX}, ${bombY}): Found ${bombsToDetonate.length} bombs to detonate:`, bombsToDetonate);
        } else if (bombType === 'diamond') {
            // Detonate all existing bombs on the board, including both carbon and diamond bombs, without any existence checks
            bombsToDetonate = [...initialBombs];
            console.log(`Diamond bomb chain reaction: Found ${bombsToDetonate.length} bombs to detonate:`, bombsToDetonate);
        }

        // Detonate each bomb in sequence, ensuring no new bombs are created or detonated automatically, without strict existence checks
        for (const bomb of bombsToDetonate) {
            // Use a lenient check to ensure bombs aren’t skipped due to state changes during cascades or clears
            if (initialBombs.some(b => b.x === bomb.x && b.y === bomb.y)) {
                console.log(`Chain detonating ${bomb.type} bomb at (${bomb.x}, ${bomb.y})`);
                if (bomb.type === 'carbon') {
                    this.score += this.bonusScores.carbonDetonation;
                    await this.clearRowAndColumn(bomb.x, bomb.y, false); // Don’t chain further during grand finale
                } else if (bomb.type === 'diamond') {
                    this.score += this.bonusScores.diamondDetonation;
                    await this.clearBoard(bomb.x, bomb.y, false); // Don’t chain further during grand finale
                }
                document.getElementById('score').textContent = `Score: ${this.score}`;
                await new Promise(resolve => setTimeout(resolve, 200)); // Small delay between chain detonations
            } else {
                console.warn(`Bomb at (${bomb.x}, ${bomb.y}) not found in initial bombs list`);
            }
        }
    }
    ...
    async clearRowAndColumn(x, y, isEndGame = false) {
        ...
        // Cascade and render, but don’t detonate new bombs automatically
        this.showerTiles();
        await this.cascadeTiles();
        this.renderBoard();

        return newBombs; // Return new bombs for tracking, but don’t detonate
    }
