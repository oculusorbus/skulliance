...
/* Loss-only flourish -- death.webp, a SEPARATE, bigger animated webp from
   the Joker-flip flash's own art (joker.webp): the flash modal's 91x128
   is right for its own cramped context, but this screen has nothing else
   competing for space, so it gets its own asset (source encoded at
   182x256, this being its actual CSS display size, not a retina
   multiplier) rather than stretching the small one up. Same asset/size
   as Crypt Conquest's own loss screen -- see .cq-result-image there.
   Slotted into the icon/title/sub pop-in sequence at .25s. */
.cc-result-image { margin: 4px auto 12px; width: 182px; height: 256px; position: relative;
