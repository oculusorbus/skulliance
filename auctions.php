...
$auction_history  = getRecentAuctionHistory($conn, 30);
...
<div class="row" id="row1">
  <div class="main">
    <h2>Auctions</h2>
    <div class="content">
      <p style="font-size:0.85rem;opacity:0.6;margin-bottom:16px;">Bid on NFTs/FTs with your points. Outbid others to claim the prize when the auction closes.</p>
      ...
      <div class="auctions-grid">
        <?php foreach ($auctions as $a):
          ...
          $ended       = !$upcoming && strtotime($a['end_date']) <= $now_ts;
          $in_delivery = !empty($a['processing']);
        ?>
        <div class="auction-card">
          ...
          <div class="auction-card-body">
            ...
            <div class="auction-stats-box">
              ...
              <div class="auction-timer">
                <?php if ($in_delivery): ?>
                <span style="color:#00c8a0;">Winner selected — NFT delivery in progress</span>
                <?php elseif ($ended): ?>
                <span style="color:#ff9600;">Ended <?php echo date('M j', strtotime($a['end_date'])); ?> — draw pending</span>
                <?php elseif ($upcoming): ?>
            ...
            <?php if ($in_delivery):
              ...
              $d_winner_id  = intval($a['winner_id'] ?? 0);
              $d_is_winner  = $sess_uid === $d_winner_id;
              $d_winner_nm  = htmlspecialchars($a['winner_name'] ?? 'Unknown');
              $d_addr       = $a['winner_address'] ?? '';
              $d_short_addr = $d_addr ? substr($d_addr, 0, 12) . '…' . substr($d_addr, -8) : '';
              $d_asset      = $a['asset_id'] ?? '';
              $d_short_asset= $d_asset ? substr($d_asset, 0, 10) . '…' . substr($d_asset, -6) : '';
              $d_days_left  = max(0, ceil(30 - (time() - strtotime($a['end_date'])) / 86400));
            ?>
            <?php if ($is_owner): ?>
            ...
            <div style="background:rgba(0,200,160,0.07);border:1px solid rgba(0,200,160,0.2);border-radius:6px;padding:9px 11px;font-size:0.78rem;display:flex;flex-direction:column;gap:5px;">
              <div style="font-weight:bold;color:#00c8a0;">&#x1F3C6; You won this auction!</div>
              <?php if ($d_asset): ?>
              <div style="display:flex;justify-content:space-between;gap:6px;">
                <span style="opacity:0.5;">Asset</span>
                <span style="font-family:monospace;font-size:0.72rem;" title="<?php echo htmlspecialchars($d_asset); ?>"><?php echo htmlspecialchars($d_short_asset); ?></span>
          ...
          <div class="auction-card-footer">
            <button class="small-button" onclick="openBidModal(<?php echo $a['id']; ?>)" style="width:100%;"><?php echo ($ended || $in_delivery) ? 'View Details' : 'View &amp; Bid'; ?></button>
            <?php if ($is_owner && !$a['completed'] && !$a['canceled'] && !$in_delivery): ?>
            <?php if (!$ended): ?>
      ...
      <div style="margin-top:28px;">
        <button id="auction-history-toggle" onclick="(function(btn){var l=document.getElementById('auction-history-list');var open=l.style.display!=='none';l.style.display=open?'none':'block';btn.textContent=(open?'\u25bc':'\u25b2')+' Auction History (<?php echo count($auction_history); ?>)'})(this)" style="background:none;border:none;color:rgba(200,216,228,0.5);font-size:0.8rem;cursor:pointer;padding:0;letter-spacing:0.02em;">&#x25BC; Auction History (<?php echo count($auction_history); ?>)</button>
        <div id="auction-history-list" style="display:none;margin-top:10px;">
          <div style="display:flex;flex-direction:column;gap:4px;">
          <?php foreach ($auction_history as $h):
...
<div class="auction-modal" id="edit-auction-modal">
  ...
  <div class="auction-modal-box">
    ...
    <div class="form-section-label">Item Details</div>
    <div class="form-row"><label>Cardano Asset ID *</label><div style="display:flex;gap:6px;"><input type="text" id="ae-asset-id" maxlength="44" placeholder="asset1..." style="flex:1;" /><button type="button" onclick="aeAssetLookup.triggerLookup()" title="Re-query blockchain" style="background:rgba(0,200,160,0.1);border:1px solid rgba(0,200,160,0.25);border-radius:6px;color:#00c8a0;padding:0 10px;font-size:1rem;cursor:pointer;flex-shrink:0;">↺</button></div></div>
...
var aAssetLookup  = null;
var aeAssetLookup = null;
document.addEventListener('DOMContentLoaded', function() {
  aAssetLookup  = setupAssetLookup('a-asset-id',  'a-image',  'a-img-status',  'a-img-preview',  'a-img-preview-img',  'a-ipfs-url',  'a-title',  'a-qty-row',  'a-title-row',  'a-image-row');
  aeAssetLookup = setupAssetLookup('ae-asset-id', 'ae-image', 'ae-img-status', 'ae-img-preview', 'ae-img-preview-img', 'ae-ipfs-url', 'ae-title', 'ae-qty-row', null, null);
