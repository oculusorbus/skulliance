...
function getTotalMissions($conn){
	$sql = "SELECT (SELECT COUNT(success_missions.id) FROM missions AS success_missions INNER JOIN users AS success_users ON success_users.id = success_missions.user_id WHERE success_missions.status = '1' AND success_users.id = users.id) AS success, 
	        ...
	        COUNT(missions.id) AS total, users.id AS user_id, missions.date_created AS missions_date
		    FROM users INNER JOIN missions ON missions.user_id = users.id INNER JOIN quests ON quests.id = missions.quest_id WHERE users.id = '".$_SESSION['userData']['user_id']."' AND DATE(missions_date) >= DATE_FORMAT(CURDATE(),'%Y-%m-01')";
	$result = $conn->query($sql);
