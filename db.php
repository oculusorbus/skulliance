...
function createItem($conn, $name, $image_url, $price, $quantity, $project_id, $override=0){
	...
	VALUES ('".mysqli_real_escape_string($name)."', '".$image_url."', '".$price."', '".$quantity."', '".$project_id."', '".$override."')";
...
function updateItem($conn, $item_id, $name, $image_url, $price, $quantity, $project_id){
	$sql = "UPDATE items SET name='".mysqli_real_escape_string($name)."', image_url='".$image_url."', price='".$price."', quantity='".$quantity."' WHERE id='".$item_id."'";
	if ($conn->query($sql) === TRUE) {
	  //echo "New record created successfully";
	} else {
	  //echo "Error: " . $sql . "<br>" . $conn->error;
