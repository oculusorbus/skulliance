...
function getInventory($conn, $project_id, $quest_id) {
	...
	if ($result->num_rows > 0) {
		...
		foreach($consumables AS $id => $consumable){
			echo "<li class='role'><input type='button' class='small-button' value='Select' onclick='processConsumable(this.value, ".$id.");' />&nbsp;".$consumable["name"]." - ".$consumable["amount"]."</li>";
		}
...
function getCurrentAmounts($conn){
	if(isset($_SESSION['userData']['user_id'])){
		$sql = "SELECT amount, name, consumables.id AS consumable_id FROM amounts INNER JOIN consumables ON amounts.consumable_id = consumables.id WHERE user_id = '".$_SESSION['userData']['user_id']."' ORDER BY rate ASC";
		$result = $conn->query($sql);
...
function checkMissionsLeaderboard($conn, $monthly=false, $rewards=false){
	...
	if ($result->num_rows > 0) {
		...
		while($row = $result->fetch_assoc()) {
			...
			$missions[$index]["score"] = round(((($row["total"]+($row["success"]*2))-($row["failure"]/2))-$row["progress"])+1);
			$index++;
		}
