...
function checkActivityLeaderboard($conn, $period = 'ath') {
	...
	if ($period === 'monthly') {
		...
		$w_m  = "AND created_date  >= '$ws'";
		$w_ge = "AND ge.resolved_date >= '$ws'";
		$w_r  = "AND r.created_date >= '$ws'";
		$w_e  = "AND date_created  >= '$ws'";
	...
	$sources = [
		...
		// Completed delves only (won or lost -- an abandon is recorded as a
		// loss, same as everywhere else Crypt Crawl counts "completed"), not
		// every in-progress row -- matches checkCryptCrawlLeaderboard()'s own
		// definition and avoids rewarding Activity points for runs someone
	...
	uasort($activity, function($a, $b) { return $b['total_pts'] - $a['total_pts']; });

	// Fetch user details for everyone who appears
	$uid_list = implode(',', array_keys($activity));
	$users_r  = $conn->query("SELECT id, username, discord_id, avatar, visibility FROM users WHERE id IN ($uid_list)");
	$users    = [];
	while ($u = $users_r->fetch_assoc()) $users[intval($u['id'])] = $u;

	$result = true; // signal to rendering block below
	...
	foreach ($activity as $uid => $data) {
		if (!isset($users[$uid])) continue;
		$u     = $users[$uid];
		$score = intval($data['total_pts']);
		$leaderboardCounter++;
...
// ── CRYPT ──────────────────────────────────────────────────
// Resurrection duration in hours: (11 - level) days, capped at level 10. Fast Forward halves it.
function getCryptResurrectionHours($conn, $realm_id) {
	$realm_id    = intval($realm_id);
	$crypt_level = min(intval(getRealmLocationLevel($conn, $realm_id, 6)), 10);
	$hours       = (11 - $crypt_level) * 24;
	if (hasLocationFastForward($conn, $realm_id, 6)) $hours = $hours / 2;
	return $hours;
}
...
function getCryptSoldiers($conn, $realm_id) {
	...
	$sql = "SELECT soldiers.id AS soldier_id, soldiers.nft_id, soldiers.dead, soldiers.weapon_id, soldiers.armor_id,
	               ...
	               DATE_ADD(soldiers.dead, INTERVAL $res_hours HOUR) AS ready_at,
	               (DATE_ADD(soldiers.dead, INTERVAL $res_hours HOUR) <= NOW()) AS eligible,
	               weapons.name AS weapon_name, weapons.level AS weapon_level,
	               armor.name AS armor_name, armor.level AS armor_level
	        FROM soldiers
	        INNER JOIN nfts ON nfts.id = soldiers.nft_id
...
function resurrectSoldiers($conn, $realm_id) {
	...
	$conn->query("UPDATE soldiers SET dead = NULL, location = 1, trained = 1 WHERE realm_id = $realm_id AND dead IS NOT NULL AND active = 1 AND DATE_ADD(dead, INTERVAL $res_hours HOUR) <= NOW()");
}
...
function releaseRaidSoldiers($conn, $raid_id) {
	...
	while ($row = $result->fetch_assoc()) {
		...
		if (intval($s['weapon_id']) > 0) {
			...
			if ($wc <= 0) { $set[] = 'weapon_id = 0'; $set[] = 'weapon_count = 0'; $weapon_degraded = 1; }
			else            $set[] = "weapon_count = $wc";
		}
		if (intval($s['armor_id']) > 0) {
			$ac = intval($s['armor_count']) - 1;
			if ($ac <= 0) { $set[] = 'armor_id = 0'; $set[] = 'armor_count = 0'; $armor_degraded = 1; }
			else            $set[] = "armor_count = $ac";
		}
		$conn->query("UPDATE soldiers SET " . implode(', ', $set) . " WHERE id = $sid LIMIT 1");
		if ($weapon_degraded || $armor_degraded) {
			$conn->query("UPDATE raids_soldiers SET weapon_degraded = $weapon_degraded, armor_degraded = $armor_degraded WHERE raid_id = $raid_id AND soldier_id = $sid AND side = 'offense' LIMIT 1");
		}
	}
}
...
function getRaidLogsDisplay($conn, $raid_id) {
	...
	// Gear degraded to destruction (offense only)
	$gresult = $conn->query("SELECT nfts.name AS nft_name, rs.weapon_degraded, rs.armor_degraded, weapons.name AS weapon_name, weapons.level AS weapon_level, armor.name AS armor_name, armor.level AS armor_level FROM raids_soldiers rs INNER JOIN soldiers ON soldiers.id = rs.soldier_id INNER JOIN nfts ON nfts.id = soldiers.nft_id LEFT JOIN weapons ON weapons.id = rs.weapon_id LEFT JOIN armor ON armor.id = rs.armor_id WHERE rs.raid_id = $raid_id AND rs.side = 'offense' AND (rs.weapon_degraded = 1 OR rs.armor_degraded = 1)");
...
define('GAUNTLET_C_75',     2); // 75%  Success
define('GAUNTLET_C_50',     3); // 50%  Success
define('GAUNTLET_C_25',     4); // 25%  Success
define('GAUNTLET_C_FF',     5); // Fast Forward — swap card
define('GAUNTLET_C_DOUBLE', 6); // Double Rewards
define('GAUNTLET_C_RANDOM', 7); // Random Reward

// Weekly reset cadence mirrors Skull Swap / Boss Battles: Thursday 4pm CST
function gauntletGetWeekStart() {
	$tz   = new DateTimeZone('America/Chicago');
	$now  = new DateTime('now', $tz);
	$dow  = intval($now->format('N')); // 1=Mon … 4=Thu … 7=Sun
	$back = ($dow - 4 + 7) % 7;
	$start = clone $now;
	if ($back > 0) $start->modify("-{$back} days");
	$start->setTime(16, 0, 0);
	if ($start > $now) $start->modify('-7 days'); // Thursday before 4pm
	$start->setTimezone(new DateTimeZone('UTC'));
	return $start->format('Y-m-d H:i:s');
}

// Returns the effective project ID used in the win-chance formula:
//   Core projects 1-6  → their own project_id
//   Diamond Skull (15) → 0  (forces 50/50)
//   Partner (8+, ≠15)  → random core id 1-6 (wildcard)
//   Anything else      → treated as partner
function gauntletGetEffectiveProjectId($project_id) {
	$pid = intval($project_id);
	if ($pid >= 1 && $pid <= 6) return $pid;
	if ($pid === 15)             return 0;
	return rand(1, 6);
}
...
// Active run = most recent run that is not over
function gauntletGetActiveRun($conn, $user_id) {
	$run = gauntletGetMostRecentRun($conn, $user_id);
	if (!$run)                                      return null;
	if (gauntletIsRunOver($conn, intval($run['id']))) return null;
	return $run;
}

// Wins / losses / total resolved encounters for a run
function gauntletGetRunStats($conn, $run_id) {
	$rid = intval($run_id);
	$r   = $conn->query("
		SELECT
			COUNT(*) AS total,
...
function gauntletGetHand($conn, $run_id) {
	...
	$r = $conn->query("
		...
		WHERE grn.run_id = $rid
		ORDER BY grn.id ASC
	");
	$hand = [];
	if ($r) while ($row = $r->fetch_assoc()) {
		$row['played'] = in_array(intval($row['id']), $played);
		$hand[] = $row;
	}
	return $hand;
}

// Pending encounter for a run with full NFT / project / user data
function gauntletGetPendingEncounter($conn, $run_id) {
	$rid = intval($run_id);
	$r   = $conn->query("
		SELECT ge.*,
		       grn.effective_project_id AS player_effective_project_id,
		       pn.name      AS player_nft_name,  pn.ipfs AS player_ipfs,  pn.collection_id AS player_collection_id,
		       on2.name     AS opponent_nft_name, on2.ipfs AS opponent_ipfs, on2.collection_id AS opponent_collection_id,
		       pc.project_id AS player_project_id,
		...
		WHERE ge.run_id = $rid AND ge.outcome = 'pending'
		LIMIT 1
	");
	if ($r && $r->num_rows) return $r->fetch_assoc();
	return null;
...
// Random opponent NFT from another staker; avoids projects already faced this run
function gauntletSelectOpponent($conn, $user_id, $run_id) {
	$uid = intval($user_id);
	$rid = intval($run_id);
	$used_projects = [];
	$r = $conn->query("
		...
		WHERE ge.run_id = $rid
	");
	if ($r) while ($row = $r->fetch_assoc()) $used_projects[] = intval($row['project_id']);
	$proj_exclude = $used_projects ? 'AND c.project_id NOT IN (' . implode(',', $used_projects) . ')' : '';
	// Try unique project first
	$r = $conn->query("
...
// Player picks an NFT from their hand — creates a pending encounter
function gauntletStartEncounter($conn, $user_id, $run_id, $nft_id) {
	$uid    = intval($user_id);
	$rid    = intval($run_id);
	$nft_id = intval($nft_id);
	...
	$opp_eff      = gauntletGetEffectiveProjectId(intval($opponent['project_id']));
...
function gauntletFastForward($conn, $user_id, $encounter_id, $new_nft_id) {
	$uid    = intval($user_id);
...
function gauntletResolveEncounter($conn, $user_id, $encounter_id, $consumable_ids = [], $weapon_id = 0, $armor_id = 0) {
	...
	if ($weapon_id > 0 && getCurrentGear($conn, $uid, 'weapon', $weapon_id) > 0) {
		$wr = $conn->query("SELECT level FROM weapons WHERE id=$weapon_id LIMIT 1");
		if ($wr && $wr->num_rows) $win_chance = min(100, $win_chance + intval($wr->fetch_assoc()['level']));
		updateGear($conn, $uid, 'weapon', $weapon_id, -1);
	} else {
		$weapon_id = 0;
	}
	// Apply armor (+level% to win chance — defensive equivalent)
	if ($armor_id > 0 && getCurrentGear($conn, $uid, 'armor', $armor_id) > 0) {
		$ar = $conn->query("SELECT level FROM armor WHERE id=$armor_id LIMIT 1");
		if ($ar && $ar->num_rows) $win_chance = min(100, $win_chance + intval($ar->fetch_assoc()['level']));
		updateGear($conn, $uid, 'armor', $armor_id, -1);
	} else {
		$armor_id = 0;
	}
	// Roll
	...
	$or2 = $conn->query("SELECT c.project_id FROM nfts n INNER JOIN collections c ON c.id=n.collection_id WHERE n.id=".intval($enc['opponent_nft_id'])." LIMIT 1");
	$opponent_project_id = ($or2 && $or2->num_rows) ? intval($or2->fetch_assoc()['project_id']) : 0;
	// Random reward swaps the reward project to a random core project
	if ($random_reward) {
		$rand_proj = $conn->query("SELECT id FROM projects WHERE id BETWEEN 1 AND 6 ORDER BY RAND() LIMIT 1");
		if ($rand_proj && $rand_proj->num_rows) {
			$rp = intval($rand_proj->fetch_assoc()['id']);
			if ($outcome === 'win') $opponent_project_id = $rp;
	...
	$wh_player_img_raw = !empty($wh_pnft) ? getIPFS($wh_pnft['ipfs'], $wh_pnft['collection_id'], $wh_pnft['project_id']) : '';
	$wh_player_img     = str_starts_with($wh_player_img_raw, '/staking/') ? 'https://skulliance.io' . $wh_player_img_raw : '';
	// Opponent NFT + user
	...
	$wh_opp_img_raw = !empty($wh_onft) ? getIPFS($wh_onft['ipfs'], $wh_onft['collection_id'], $wh_onft['project_id']) : '';
	$wh_opp_img     = str_starts_with($wh_opp_img_raw, '/staking/') ? 'https://skulliance.io' . $wh_opp_img_raw : '';
	...
	$wh_wins     = intval($wh_stats['wins']);
	$wh_swept    = ($outcome === 'win' && $wh_wins >= GAUNTLET_MAX_WINS);
	if ($outcome === 'win') {
		if ($wh_swept) {
			$wh_title = "🏆 Gauntlet Swept!";
			$wh_intro = $wh_mention . " swept the Gauntlet — undefeated!\n\n";
			$wh_color = "FFD700";
		} else {
			$wh_title = "⚔️ Gauntlet Victory!";
			$wh_intro = $wh_mention . " defeated " . $wh_opp_mention . "!\n\n";
			$wh_color = "00C8A0";
		}
		$wh_desc  = $wh_intro;
		$wh_desc .= "🦴 **Player:** " . ($wh_pnft['name'] ?? 'Unknown') . " (" . ($wh_pnft['project_name'] ?? '') . ")\n";
		$wh_desc .= "💀 **Defeated:** " . ($wh_onft['name'] ?? 'Unknown') . " (" . ($wh_onft['project_name'] ?? '') . ") — [$wh_opp_uname]($wh_opp_profile)\n";
		$wh_desc .= "🎯 **Odds:** $win_chance%\n";
		...
		$wh_desc .= "🦴 **Player:** " . ($wh_pnft['name'] ?? 'Unknown') . " (" . ($wh_pnft['project_name'] ?? '') . ")\n";
		$wh_desc .= "💀 **Victor:** " . ($wh_onft['name'] ?? 'Unknown') . " (" . ($wh_onft['project_name'] ?? '') . ") — [$wh_opp_uname]($wh_opp_profile)\n";
		$wh_desc .= "🎯 **Odds:** $win_chance%\n";
...
/* ============================================================
   ...
   Requires table `cryptcrawls` (see skullpaper/MAINTENANCE.md
...
function cryptcrawlWeaponName($conn, $power) {
...
// Cheap average-color read for a locally-cached NFT image: resize to 1x1
// with GD's resampler (a well-known fast way to approximate average color)
// and read the resulting pixel. Only works on our own locally-cached files
// under /staking/images/nfts/ (same convention as image.php elsewhere in
// this codebase), not remote IPFS-gateway fallback URLs — returns null for
// anything it can't read locally or decode, so a card's frame just falls
// back to the default gradient rather than breaking the page.
function cryptcrawlDominantColor($web_url) {
	$prefix = '/staking/images/nfts/';
	if (strpos($web_url, $prefix) !== 0) return null;
	$rel  = strtok(substr($web_url, strlen($prefix)), '?'); // drop ?v= cache-buster
	$path = __DIR__ . '/images/nfts/' . $rel;
	if (!is_file($path)) return null;

	$type = @exif_imagetype($path);
	if ($type === IMAGETYPE_JPEG)      $img = @imagecreatefromjpeg($path);
	elseif ($type === IMAGETYPE_PNG)   $img = @imagecreatefrompng($path);
	elseif ($type === IMAGETYPE_GIF)   $img = @imagecreatefromgif($path);
	elseif (function_exists('imagecreatefromwebp') && $type === IMAGETYPE_WEBP) $img = @imagecreatefromwebp($path);
	else return null;
	if (!$img) return null;

	$tiny = imagecreatetruecolor(1, 1);
	imagecopyresampled($tiny, $img, 0, 0, 0, 0, 1, 1, imagesx($img), imagesy($img));
...
function cryptcrawlRefillRoom(&$run) {
...
function cryptcrawlPlayCard($conn, $run_id, $card_index, $use_weapon) {
	...
	if ($card['type'] === 'weapon') {
		...
		$run['weapon_name']        = cryptcrawlWeaponName($conn, intval($card['rank']));
...
// entry just falls back to a plain card face).
function cryptconquestZipArtPool($pool, $keys, $offset) {
	$art = [];
	foreach ($keys as $i => $key) {
		$pool_index = $offset + $i;
		if (!isset($pool[$pool_index])) break;
		$art[$key] = $pool[$pool_index];
	}
	return $art;
}
