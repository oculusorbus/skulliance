	...
	if ($result->num_rows > 0) {
	  ...
	  while($row = $result->fetch_assoc()) {
		...
		if($row["collection_id"] == 4 || $row["collection_id"] == 23){
			...
			echo "<span class='nft-image'><img src='https://storage.googleapis.com/jpeg-optim-files/".str_replace("ipfs/", "", $row["ipfs"])."'/></span>";
		}else{
...
function checkLeaderboard($conn, $clean, $project_id=0) {
	$sql = "SELECT nfts.id, nfts.user_id, SUM(nfts.id) as total, users.username, users.discord_id AS discord_id, avatar, projects.id AS project_id FROM nfts INNER JOIN users ON nfts.user_id=users.id INNER JOIN collections ON collections.id = nfts.collection_id INNER JOIN projects ON projects.id = collections.project_id WHERE collections.project_id = '".$project_id."' GROUP BY results.user_id ORDER BY total DESC";
	$result = $conn->query($sql);
