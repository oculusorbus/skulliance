...
function getMissions($conn, $quest_id) {
	...
	$sql = "SELECT quests.id, title, description, cost, reward, project_id, duration, level, currency, name FROM quests INNER JOIN projects ON projects.id = quests.project_id ORDER BY CASE WHEN projects.id = '".$project_id."' THEN 0 ELSE 1 ASC, projects.id ASC";
