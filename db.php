...
function getRaids($conn, $type){
	if(isset($_SESSION['userData']['user_id'])){
		...
		$sql = "SELECT raids.id AS raid_id, realms.name AS realm_name, theme_id, raids.duration AS duration, raids.created_date AS created_date, username, discord_id, avatar 
			    FROM raids INNER JOIN realms ON realms.id = raids.".$id1." INNER JOIN users ON users.id = realms.user_id WHERE ".$id2." = '".$realm_id."' ORDER BY raids.id ASC THEN BY outcome ASC LIMIT 10";
		$result = $conn->query($sql);
