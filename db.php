...
function getLocationInfo($conn){
	$sql = "SELECT name, description, project_id FROM locations";
	$result = $conn->query($sql);
...
function getRealms($conn){
	...
	$sql = "SELECT DISTINCT realms.id AS realm_id, realms.name AS realm_name, users.id AS user_id, users.username AS username, users.avatar AS avatar, users.discord_id AS discord_id, CASE WHEN outcome IS NULL THEN 'NONE' ELSE outcome END AS outcome 
		    FROM realms INNER JOIN users ON users.id = realms.user_id LEFT JOIN raids ON raids.destination_id = realms.id WHERE raids.outcome = '0' OR raids.outcome ='NONE' GROUP BY realms.id";
