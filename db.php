...
function getCurrentMissions($conn){
	...
	if ($result->num_rows > 0) {
	  ...
	  while($row = $result->fetch_assoc()) {
		...
		echo "<div class='w3-grey' style='width:".$percentage."%;background-image:url("."images/missions/".strtolower(str_replace(" ", "-", $row["title"])).".png".");background-position:center;opacity:0.5;background-position:fixed'></div>";
		echo "</div>";
		echo "</td>";
		echo "</tr>";
	  }
