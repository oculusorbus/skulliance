...
function checkMissionsLeaderboard($conn){
	$sql = "SELECT (SELECT COUNT(success_missions.id) FROM missions AS success_missions WHERE success_missions.status = '1') AS success, (SELECT COUNT(failed_missions.id) FROM missions AS failed_missions WHERE failed_missions.status = '2') AS failure, COUNT(missions.id) AS total, users.id AS user_id, username, avatar, discord_id, visibility 
