...
function getCurrentMissions($conn){
	...
	if ($result->num_rows > 0) {
	  ...
	  while($row = $result->fetch_assoc()) {
		...
		echo "<tr id='mission-row-".$row["mission_id"]."' style='background-image:url(\"images/missions/".strtolower(str_replace(" ", "-", $row["title"])).".png\");background-repeat:no-repeat;background-size:cover;background-position:center;background-blend-mode: multiply;' >";
