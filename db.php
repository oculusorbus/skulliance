...
function checkMissionsLeaderboard($conn){
	$sql = "SELECT (SELECT COUNT(m.id) FROM missions AS m) AS success, (COUNT(l.id) FROM missions AS l) AS failure, COUNT(missions.id) AS total, users.id AS user_id, username, avatar, discord_id, visibility
