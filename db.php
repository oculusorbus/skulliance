...
function checkMissionsLeaderboard($conn){
	$sql = "SELECT (SELECT COUNT(id) FROM missions AS successful_missions) AS success, (COUNT(id) FROM missions AS failed_missions) AS failure, COUNT(missions.id) AS total, users.id AS user_id, username, avatar, discord_id, visibility, 
