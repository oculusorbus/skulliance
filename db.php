...
function getRealms($conn){
	if(isset($_SESSION['userData']['user_id'])){
		...
		if ($result->num_rows > 0) {
			while($row = $result->fetch_assoc()) {
				echo "<th align='left'>".$row['realm_name']."</th><th align='left'>".$row["username"]."</th><th align='left'>Location Levels</th><th align='left'>Top Points Balances</th>";
				echo "<tr>";
				...
				if(checkMaxRaids($conn, $offense_id)){
					if(checkRealmRaidStatus($conn, $row["realm_id"])){
						...
						echo "<input type='button' class='small-button' value='".$value."' onclick='startRaid(this, ".$row['realm_id'].", ".$duration.");'><br><br>";
					}else{
				...
				foreach($levels AS $location_name => $level){
					echo "<tr>";
					echo "<td>".ucfirst($location_name).":</td>";
					echo "<td>".$level."</td>";
					echo "</tr>";
				}
				echo "</table>";
