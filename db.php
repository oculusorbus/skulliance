	...
	if ($result->num_rows > 0) {
	  ...
	  while($row = $result->fetch_assoc()) {
		...
		if($row["collection_id"] == 4 || $row["collection_id"] == 23){
			echo "<span class='nft-image'><img onError='this.src=\"/drop-ship/icons/xp.png\";' src='https://ipfs2.jpgstoreapis.com/".$row["ipfs"]."'/></span>";
		}else if($row["collection_id"] == 20 || $row["collection_id"] == 21 || $row["collection_id"] == 30){
			echo "<span class='nft-image'><img onError='this.src=\"/drop-ship/icons/xp.png\";' src='https://storage.googleapis.com/jpeg-optim-files/".str_replace("ipfs/", "", $row["ipfs"])."'/></span>";
		}else{
			echo "<span class='nft-image'><img onError='this.src=\"/drop-ship/icons/xp.png\";' src='https://image-optimizer.jpgstoreapis.com/".$row["ipfs"]."'/></span>";
		}
