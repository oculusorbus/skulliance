...
function getMissions($conn, $quest_id) {
	$sql = "SELECT quests.id, title, description, cost, reward, project_id, duration, currency, name FROM quests INNER JOIN projects ON projects.id = quests.project_id ORDER BY CASE WHEN quests.id = '".$quest_id."' THEN 1 ELSE 2 END, quests.id";
...
function getInventory($conn, $project_id, $quest_id) {
	...
	$sql = "SELECT SUM(rate), nft_id AS total_mission_rates FROM missions_nfts INNER JOIN missions ON missions.id = missions_nfts.mission_id INNER JOIN quests ON quests.id = missions.quest_id INNER JOIN nfts ON nfts.id = missions_nfts.nft_id INNER JOIN collections ON collections.id = nfts.collection_id WHERE status = '0' AND missions.user_id = '".$_SESSION['userData']['user_id']."' AND quests.project_id = '".$project_id."'";
		...
		if($total_rates > 100){
			echo "<li class='role'>
				  <form action='missions.php' method='post'>
					<input type='button' class='small-button' value='Maximize Inventory Selections for Single Mission    '>Maximize Inventory Selections for Single Mission</input>
					<input type='button' class='small-button' value='Balance Inventory Selections for Multiiple Missions'/></input>
				  </form>
				  </li>";
		}
...
function completeMission($conn, $mission_id, $quest_id){
	if(isset($_SESSION['userData']['user_id'])){
		...
		if($success == 1){
			...
			echo $title." Mission was successful!\r\n".$reward." ".$currency." has been added to your balance and transaction history.";
		}else if($success == 2){
