...
function checkMissionsLeaderboard($conn){
	$sql = "SELECT COUNT(missions.id) AS total, users.id AS user_id, username, avatar, discord_id, visibility, 
		    (SELECT COUNT(status) FROM missions INNER JOIN users ON users.id = missions.user_id WHERE missions.status = '1' GROUP BY users.id) AS success, (COUNT(status) FROM missions INNER JOIN users ON users.id = missions.user_id WHERE missions.status = '2' GROUP BY users.id) AS failure 
