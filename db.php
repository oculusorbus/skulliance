...
function selectRandomProjectID($conn, $realm_id){
	$sql = "SELECT project_id, balance FROM realms INNER JOIN users ON users.id = realms.user_id INNER JOIN balances ON balances.user_id = users.id WHERE realms.id = '".$realm_id."'";
	$result = $conn->query($sql);
