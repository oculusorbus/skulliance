...
function getTotalNFTs($conn, $project_id=0){
	...
	$sql = "SELECT COUNT(users.id) as total FROM nfts INNER JOIN users ON nfts.user_id=users.id INNER JOIN collections ON collections.id = nfts.collection_id INNER JOIN projects ON projects.id = collections.project_id ".$inner_join.$where." AND nfts.user_id != '0'";
	$result = $conn->query($sql);
	...
	if ($result->num_rows > 0) {
	  // output data of each row
		...
		// Formatted output for website leaderboard
			...
			echo "<th>#</th><th>Rank</th><th>Avatar</th><th align='left'>Username</th><th>NFTs</th><th>Points</th>";
		  	while($row = $result->fetch_assoc()) {
				$rowCounter++;
				$leaderboardCounter++;
				$width = 40;
		    	...
		    	echo "<td align='center'>".$rowCounter."</td><td align='center'><strong>".(($trophy == "")?(($leaderboardCounter<10)?"0":"").$leaderboardCounter.".":$trophy)."</strong></td><td align='center'>".$avatar."</td><td><strong style='font-size:20px'>".$username."</strong></td><td align='center'>".$row["total"].$delegated."</td><td align='center'>".(($project_id != 0)?" ".number_format($current_balance)." ".$row["currency"]."":"").$diamond_skull_count."</td>";
				echo "</tr>";
