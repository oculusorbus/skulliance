	...
	if ($result->num_rows > 0) {
		while($row = $result->fetch_assoc()) {
			if($last_realm_id != $row['realm_id']){
				...
				if($row["avatar"] != ""){
					echo "<img style='width:25px' onError='this.src=\"/staking/icons/skull.png\";' src='https://cdn.discordapp.com/avatars/".$row["discord_id"]."/".$row["avatar"].".jpg' class='icon rounded-full'/>";
				}
