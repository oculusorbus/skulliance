...
function getCurrentMissions($conn){
	...
	if ($result->num_rows > 0) {
	  ...
	  echo "<th align='center' width='55'>Icon</th><th width='55' align='center'>Project</th><th align='left'>Cost</th><th align='left'>Reward</th><th align='left'>NFTs</th><th align='left'>Success</th><th align='left'>Time Left</th><th align='left' id='consumable-header'>Items</th><th align='center'>Status</th>";
	  // output data of each row
	  while($row = $result->fetch_assoc()) {
...
function getMissionConsumables($conn, $mission_id){
	$sql = "SELECT consumables.id AS consumable_id, name FROM consumables INNER JOIN missions_consumables ON consumables.id = missions_consumables.consumable_id WHERE missions_consumables.mission_id = '".$mission_id."'";
	$result = $conn->query($sql);
