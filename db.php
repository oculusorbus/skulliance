...
function getRealmLocationUpgrade($conn, $realm_id, $location_id){
	...
	if ($result->num_rows > 0) {
		while($row = $result->fetch_assoc()) {
			...
			$minutes_remaining = floor(($remaining % 3600) / 60);
			if($date > time()){
				$time_message = "<strong>Current Upgrade: </strong>Level ".$row["duration"];
				$time_message .= "<strong>Current Duration: </strong>".$row["duration"]." ".(($row["duration"]==1)?"Day":"Days")."<br>";
				$time_message .= $days_remaining."d ".$hours_remaining."h ".$minutes_remaining."m";
...
function getRealms($conn){
	if(isset($_SESSION['userData']['user_id'])){
		...
		if ($result->num_rows > 0) {
			while($row = $result->fetch_assoc()) {
				echo "<th>".ucfirst($row['realm_name'])."</th><th>Raid Details</th><th>Location Levels</th><th>Top Points Balances</th>";
				echo "<tr>";
				...
				if(checkMaxRaids($conn, $offense_id)){
					...
					echo "<strong>Max Raids Reached</strong><br><br>";
				}
				echo "</td>";
				echo "</tr>";
				echo "</table>";
