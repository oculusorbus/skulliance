...
function getRealms($conn){
	...
	$sql = "SELECT DISTINCT locations.id AS location_id, realms.name AS realm_name, realms.id AS realm_id, users.id AS user_id, users.username AS username, users.avatar AS avatar, users.discord_id AS discord_id, CASE WHEN outcome IS NULL THEN 3 ELSE outcome END AS outcome, realms_locations.level AS level, locations.name AS location_name   
