...
function getCurrentMissions($conn){
	$sql = "SELECT title FROM missions INNER JOIN quests ON missions.quest_id = quests.id INNER JOIN missions_nfts ON missions.id = missions_nfts.mission_id WHERE status = 0 AND user_id = '".$_SESSION['userData']['user_id']."'";
