...
function getRealms($conn, $sort){
	if(isset($_SESSION['userData']['user_id'])){
		...
		$sql = "SELECT DISTINCT realms.id AS realm_id, realms.name AS realm_name, theme_id, users.id AS user_id, users.username AS username, users.avatar AS avatar, users.discord_id AS discord_id
			    FROM realms INNER JOIN users ON users.id = realms.user_id ORDER BY rand()";
...
function getTotalRaids($conn){
	$month_sql = "SELECT (SELECT COUNT(success_raids.id) FROM raids AS success_raids INNER JOIN success_realms ON success_realms.id = success_raids.offense_id INNER JOIN users AS success_users ON success_users.id = success_realms.user_id 
	             ...
	             (SELECT COUNT(failed_raids.id) FROM raids AS failed_raids INNER JOIN failed_realms ON failed_realms.id = failed_raids.offense_id INNER JOIN users AS failed_user ON failed_users.id = failed_realms.user_id 
				 ...
				 (SELECT COUNT(progress_raids.id) FROM raids AS progress_raids INNER JOIN progress_realms ON progress_realms.id = progress_raids.offense_id INNER JOIN users AS progress_users ON progress_users.id = progress_realms.user_id 
...
function selectRandomProjectID($conn, $realm_id){
	$sql = "SELECT realms.id AS realm_id, project_id, balance FROM realms INNER JOIN users ON users.id = realms.user_id INNER JOIN balances ON balances.user_id = users.id WHERE realms.id = '".$realm_id."'";
	$result = $conn->query($sql);
