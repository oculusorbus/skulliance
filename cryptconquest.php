...
/* Indented 15px to line up with the button labels above (Play Selected /
   Yield), which sit inset from .cq-hand-controls's own 14px panel padding
   plus each .cq-btn's own left padding -- this note isn't inside that
   panel, so it needs the offset spelled out directly rather than inheriting it. */
.cq-note { font-size: 0.68rem; opacity: 0.5; margin: 8px 0 16px; padding-left: 15px; }
...
.cq-suit-key { display: flex; flex-wrap: wrap; justify-content: center; gap: 8px; margin-top: 14px; font-size: 0.7rem; }
.cq-suit-key-item {
...
/* Loss-only flourish -- the same animated Joker webp the Joker-flip flash
   uses, here as a standalone illustration rather than a card-framed
   thumbnail (no crowded card row to match on this screen, so it's allowed
   to breathe). Slotted into the icon/title/sub pop-in sequence at .25s. */
.cq-result-image { margin: 4px auto 12px; width: 180px; border-radius: 14px; overflow: hidden;
