...
Skulliance supports **multiple Cardano wallets** per account, so you can stake NFTs held across several wallets under one profile.
...
* A wallet must be **delegated to a stake pool** before it can be connected -
  Skulliance identifies a wallet by its stake address.
