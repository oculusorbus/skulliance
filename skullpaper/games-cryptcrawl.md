...
Crypt Crawl is two standalone pages (no site nav), same treatment as [[games-skull-swap]] and Monstrocity. `cryptcrawlgame.php` is the public marketing page you land on from the site nav or homepage - hero, feature highlights, two counter-scrolling rows showing every Crypties NFT actually used as card art, mechanics/tips/FAQ, and a "Start Delve" button that starts a real delve on `cryptcrawl.php`, ending in a footer (same style as Skull Swap's own) rather than a "Go Back" button - it's the front door, not a page you need an escape hatch from. `cryptcrawl.php` is the game itself: a simple rules prompt if you don't have a delve going, the live board while one's in progress, and a result screen (win/loss, crypts cleared, CARBON earned) once it ends - with a "Delve Again" button and a link back to the marketing page. Every screen on `cryptcrawl.php` has a "Go Back" button, since it has no site nav to fall back on otherwise. Logged in, it always returns you straight to your [[platform-dashboard]]; logged out, it prefers real browser history (when you arrived from elsewhere on the site) over a fixed destination, falling back to the public homepage.
