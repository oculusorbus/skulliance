...
`cryptcrawl.php` is a standalone page (no site nav), same as [[games-skull-swap]] and Monstrocity - it doubles as the game's public marketing landing. The landing (hero, feature highlights, two counter-scrolling rows showing every Crypties NFT actually used as card art, FAQ, a "Start Delve"/"Delve Again" button) shows any time you don't currently have a delve **in progress** - a first-ever visit, or any later visit once your last one has ended. Land there with a delve already active and it skips straight to the game instead; land there with your last one just finished and the landing still shows, with your result (win/loss, crypts cleared, CARBON earned) at the top of it. Every screen has a "Go Back" button, since there's no site nav on this page to fall back on otherwise - it prefers real browser history (when you arrived from elsewhere on the site) over a fixed destination.
