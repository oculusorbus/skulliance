...
if (!empty($_SESSION['userData']['discord_id'])) {
	$did = mysqli_real_escape_string($conn, $_SESSION['userData']['discord_id']);
	mysqli_query($conn, "UPDATE users SET last_login = NOW() WHERE discord_id = '$did' AND (last_login IS NULL OR last_login < NOW() - INTERVAL 1 DAY)");
}
		...
		if($carbon >= 100){
			...
			<form onsubmit="return false;" id="carbonForm" action="<?php echo $page; ?>.php" method="post">
			  ...
			  <div style="display:flex; align-items:center; gap:8px; margin-bottom:8px;">
			    ...
			    <span style="color:#888;">→</span>
			    <img class="icon" src="icons/diamond.png" style="flex-shrink:0;">
			    <span id="burn-diamond-display" style="font-weight:bold;color:#00c8a0;"><?php echo number_format($carbon_max/100); ?></span>
			  </div>
			  <div style="display:flex; align-items:center; gap:8px;">
			    <input type="range" name="carbon" id="carbon" min="100" max="<?php echo $carbon_max; ?>" step="100" value="<?php echo $carbon_max; ?>" style="flex:1;"
			      oninput="document.getElementById('carbon-display').textContent=Number(this.value).toLocaleString();document.getElementById('burn-diamond-display').textContent=Number(this.value/100).toLocaleString();">
			    <button type="button" class="small-button" style="flex-shrink:0;" onclick="confirmForm(this.form, 'Burn ' + document.getElementById('carbon-display').textContent + ' CARBON to form ' + document.getElementById('burn-diamond-display').textContent + ' DIAMOND?')">Burn</button>
			  </div>
			</form>
			</li>
			<?php
			...
			echo '<li class="role">';
			echo "You need at least 100 CARBON to form DIAMOND.<br><br>Claim daily rewards, participate in missions, or delegate your core project NFTs to Diamond Skulls to earn CARBON.";
			echo '</li>';
		}
	}
	...
	<div id="filter-nfts">
		...
		<select onchange="javascript:filterLeaderboard(this.options[this.selectedIndex].value);" name="filterLeaderboard" id="filterLeaderboard">
			...
			<optgroup label="Skull Racer">
				<option value="skullracer">All Skull Racer</option>
				<option value="weekly-skullracer">Weekly Skull Racer</option>
			</optgroup>
