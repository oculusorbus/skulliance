		...
		<div id="nfts" class="nfts">
			...
			if(isset($_SESSION['userData']['user_id'])){
				...
				if($total_pages > 1){
					...
					if($page > 1){
						echo "<a class='page-btn' href='my-nfts.php?filterby={$fb}&page=".($page-1)."'>&#8592; Prev</a>";
					}else{
					...
					for($p = 1; $p <= $total_pages; $p++){
						if($p == 1 || $p == $total_pages || ($p >= $page - 2 && $p <= $page + 2)){
							...
							if($p == $page){
								...
								echo "<a class='page-btn' href='my-nfts.php?filterby={$fb}&page={$p}'>{$p}</a>";
							}
							$prev = $p;
						}
					}
					if($page < $total_pages){
						echo "<a class='page-btn' href='my-nfts.php?filterby={$fb}&page=".($page+1)."'>Next &#8594;</a>";
					}else{
