...
if(isset($_GET['realm_id']) && isset($_GET['location_id']) && isset($_GET['duration']) && isset($_GET['cost'])){
	...
	foreach($balances AS $currency => $balance){
		if($currency != "CARBON" && $balance >= ($points_multiplier*$_GET['cost'])){
		  echo '<option onselect="javascript:document.getElementById(\'points-button-'.$_GET['location_id'].'\').value=\'Upgrade '.number_format($points_multiplier*$_GET['cost']).' '.$currency.'\';" value="'.$currency.'">'.$currency.' ('.number_format($balance).')</option>';
		}
	}
	echo '</select>';
