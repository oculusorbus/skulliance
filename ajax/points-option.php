...
if(isset($_GET['realm_id']) && isset($_GET['location_id']) && isset($_GET['duration']) && isset($_GET['cost'])){
	...
	echo '<select name="points" id="points-'.$_GET['location_id'].'" onchange="document.getElementById(\'points-button-'.$_GET['location_id'].'\').value=\'Upgrade '.number_format($points_multiplier*$_GET['cost']).' \'+this.options[this.selectedIndex].value;">';
	 echo '<option name="Points" value="0">Select Points Balance</option>';
	foreach($balances AS $project_id => $balance){
		if($balance["balance"] >= ($points_multiplier*$_GET['cost'])){
		  echo '<option name="'.$balance["currency"].'" value="'.$project_id.'">'.$balance["currency"].' ('.number_format($balance["balance"]).')</option>';
		}
	}
	echo '</select>';
