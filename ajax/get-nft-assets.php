...
function getNFTAssets($conn, $policy_ids) {
    ...
    debug_log("DB returned $index assets", $is_debug);
    return $asset_list;
}
...
if (is_array($asset_list) && !empty($asset_list["_asset_list"])) {
    ...
    foreach ($batches as $batch_index => $batch) {
        ...
        // If empty, retry with CIP-68 prefix
        if (is_array($tokenresponse) && empty($tokenresponse)) {
            debug_log("Batch $batch_index returned empty response, retrying with CIP-68 prefix", $is_debug);
        ...
        if (!is_array($tokenresponse)) {
            ...
            debug_log("Error: Invalid Koios response for batch $batch_index", $is_debug);
            continue;
        }
