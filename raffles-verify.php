...
//
// Phase 1 (processing=0, newly ended): draw winner, save winner_id, set processing=1.
//   Do NOT pay creator yet.
//   If no asset_id: pay creator from ticket proceeds immediately and mark completed (processing=0).
//
// Phase 2 (processing=1): nightly check of winner's wallet.
//   NFT confirmed → credit creator with all ticket proceeds, mark completed (processing=0).
//   30+ days elapsed → cancel, refund all ticket buyers (processing=0).
...
// ════════════════════════════════════════════════════════════════════════════
// PHASE 1 — Newly ended raffles: draw winner
// ════════════════════════════════════════════════════════════════════════════
$result = $conn->query(
...
if ($result && $result->num_rows > 0) {
    while ($raffle = $result->fetch_assoc()) {
        ...
        if ($asset_id !== '') {
            // ── NFT raffle: save winner, set processing=1 for Phase 2 tracking ─
            $conn->query("UPDATE raffles SET winner_id='$winning_uid', processing=1 WHERE id='$rid'");
    ...
    echo "No new raffles to process (Phase 1).\n";
}
...
if (!$result2 || $result2->num_rows === 0) {
    echo "No raffles in delivery tracking (Phase 2).\n";
    $conn->close();
    echo "Done.\n";
    exit;
}

while ($raffle = $result2->fetch_assoc()) {
    $rid        = intval($raffle['id']);
    $title      = $raffle['title'];
    $img_url    = !empty($raffle['image']) ? 'https://skulliance.io/staking/images/raffles/' . $raffle['image'] : '';
    $creator_id = intval($raffle['user_id']);
    $winner_id  = intval($raffle['winner_id'] ?? 0);
    $asset_id   = trim($raffle['asset_id'] ?? '');

    echo "Phase 2 — raffle #$rid: $title\n";
