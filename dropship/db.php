...
function evaluateText($text){
	...
	if(isset($_SESSION['userData']['dropship_project_id'])){
		...
		} else if($_SESSION['userData']['dropship_project_id'] == 3 || $_SESSION['userData']['dropship_project_id'] == 4){
			if($_SESSION['userData']['dropship_project_id'] == 3){
...
function createBattle($conn, $wager) {
	...
	VALUES ('".$_SESSION['userData']['dropship_user_id']."', '".$wager."', '".$_SESSION['userData']['dropship_project_id']."', '1')";

	if ($conn->query($sql) === TRUE) {
		// Remove wager from balance if db insertion is successful
	  	removeBalance($conn, $wager, $_SESSION['userData']['dropship_user_id']);
...
function deleteBattle($conn, $battle_id) {
	...
	$sql = "DELETE FROM battles WHERE id = '".$battle_id."' AND project_id = '".$_SESSION['userData']['dropship_project_id']."' AND opponent_score = '0'";
	if ($conn->query($sql) === TRUE) {
	  //echo "Record deleted successfully";
		addBalance($conn, $wager, $_SESSION['userData']['dropship_user_id']);
...
function announce($type, $user_id, $battle_id){
	...
	echo "&project_id=".$_SESSION['userData']['dropship_project_id'];
	...
	if($type == "opponent"){
		...
		if ($conn->query($sql) === TRUE) {
			...
			if($_SESSION['userData']['score'] > $opponent_score){
				addBalance($conn, ($wager*2), $_SESSION['userData']['dropship_user_id']);
...
function getBattles($conn) {
	global $prefix;
	$sql = "SELECT battles.id AS battle_id, user_id, wager, opponent_score, username, discord_id, avatar FROM battles INNER JOIN users ON users.id = battles.user_id WHERE battles.project_id = '".$_SESSION['userData']['dropship_project_id']."' AND active = '1' ORDER BY wager DESC";
	$result = $conn->query($sql);
	...
	if ($result->num_rows > 0) {
	  ...
	  while($row = $result->fetch_assoc()) {
		...
		// $row["user_id"] != $_SESSION['userData']['dropship_user_id'] && 
		echo "<td align='center'>";
		if($row["user_id"] != $_SESSION['userData']['dropship_user_id'] && $row["opponent_score"] == 0 && checkBalance($conn) >= $row["wager"] && !isset($_SESSION['userData']['battle_id'])){
			echo '<form id="opponentForm" action="dashboard.php#barracks" method="post">
			  <input type="hidden" id="opponent_id" name="opponent_id" value="'.$_SESSION['userData']['dropship_user_id'].'">
		...
		// Check if user created game, if so provide cancellation form if an opponent hasn't logged a score yet.
		else if($row["user_id"] == $_SESSION['userData']['dropship_user_id'] && $row["opponent_score"] == 0){
			...
			echo '<form id="creatorForm" action="dashboard.php#barracks" method="post">
			  <input type="hidden" id="creator_id" name="creator_id" value="'.$_SESSION['userData']['dropship_user_id'].'">
			  <input type="hidden" id="battle_id" name="battle_id" value="'.$row["battle_id"].'">
...
function getProjectName($conn) {
	$sql = "SELECT name FROM projects WHERE id = '".$_SESSION['userData']['dropship_project_id']."'";
	$result = $conn->query($sql);
	if ($result->num_rows > 0) {
	  // output data of each row
	  while($row = $result->fetch_assoc()) {
...
function getProjectPolicyId($conn) {
	$sql = "SELECT policy_id FROM projects WHERE id = '".$_SESSION['userData']['dropship_project_id']."'";
	$result = $conn->query($sql);
	$projects = array();
	if ($result->num_rows > 0) {
	  // output data of each row
	...
	if(isset($_SESSION['userData']['discord_id'])){
		...
		if ($result->num_rows > 0) {
		  ...
		  while($row = $result->fetch_assoc()) {
	    	...
	    	$_SESSION['userData']['dropship_user_id'] = strval($row["id"]);
			updateUser($conn);
		  }
		} else {
		  //echo "0 results";
...
function updateUser($conn) {
	$sql = "UPDATE users SET username='".$_SESSION['userData']['name']."', avatar='".$_SESSION['userData']['avatar']."' WHERE id='".$_SESSION['userData']['dropship_user_id']."'";
	if ($conn->query($sql) === TRUE) {
	  //echo "New record created successfully";
	} else {
	  //echo "Error: " . $sql . "<br>" . $conn->error;
...
function checkGame($conn) {
	if(isset($_SESSION['userData']['dropship_project_id'])){
		$sql = "SELECT id, active FROM games WHERE active=1 AND project_id = '".$_SESSION['userData']['dropship_project_id']."'";
		$result = $conn->query($sql);
...
function resurrectSquad($conn){
	$sql = "UPDATE soldiers SET deceased = '0', active = '0' WHERE deceased = 1 AND active = 0 AND user_id = '".$_SESSION['userData']['dropship_user_id']."' AND project_id = '".$_SESSION['userData']['dropship_project_id']."'";
	if ($conn->query($sql) === TRUE) {
...
function killSquad($conn){
	$sql = "UPDATE soldiers SET deceased = '1', active = '0' WHERE active = 1 AND user_id = '".$_SESSION['userData']['dropship_user_id']."' AND project_id = '".$_SESSION['userData']['dropship_project_id']."'";
	if ($conn->query($sql) === TRUE) {
	  //echo "Record updated successfully";
	} else {
	  //echo "Error: " . $sql . "<br>" . $conn->error;
...
function checkDefaultSoldiers($conn){
	$sql = "SELECT title, soldiers.id AS soldier_id FROM soldiers INNER JOIN users ON users.id = soldiers.user_id WHERE user_id = '".$_SESSION['userData']['dropship_user_id']."' AND project_id = '".$_SESSION['userData']['dropship_project_id']."' AND rank = 'Grunt'";
	$result = $conn->query($sql);
...
function addDefaultSoldiers($conn){
	...
	for ($x = 1; $x <= 4; $x++) {
		...
		VALUES ('GRUNT00".$x."', '".$names[$x]." #00', 'ZGRUNT', 'Grunt', 'None', 'None', '0', '".$_SESSION['userData']['dropship_project_id']."', '".$_SESSION['userData']['dropship_user_id']."', '0')";
...
function deploySoldier($conn, $id, $active){
	...
	if((checkSquadCount($conn) < 4 && $active==1) || $active==0){
		...
		$sql = "UPDATE soldiers SET active = '".$active."' WHERE id = '".$id."' AND project_id = '".$_SESSION['userData']['dropship_project_id']."'";
		if ($conn->query($sql) === TRUE) {
		  //echo "Record updated successfully";
		} else {
		  //echo "Error: " . $sql . "<br>" . $conn->error;
...
function getSoldiers($conn, $active, $filterby="", $all=false){
	...
	if(!$all){
		$user_clause = "user_id = '".$_SESSION['userData']['dropship_user_id']."' ";
	}

	$sql = "SELECT name, asset_name, ipfs, rank, armor, gear, level, username, deceased, active, soldiers.id AS soldier_id FROM soldiers INNER JOIN users ON users.id = soldiers.user_id WHERE ".$user_clause.$active_clause.$filterby." AND project_id = '".$_SESSION['userData']['dropship_project_id']."' ORDER BY deceased, title, soldiers.id";
	$result = $conn->query($sql);
...
function setSquad($conn){
	$sql = "SELECT * FROM soldiers WHERE user_id = '".$_SESSION['userData']['dropship_user_id']."' AND active = 1 AND project_id = '".$_SESSION['userData']['dropship_project_id']."' ORDER BY deceased, name";
	$result = $conn->query($sql);
...
function checkSoldier($conn, $asset_name){
	$sql = "SELECT * FROM soldiers WHERE asset_name = '".$asset_name."' AND project_id = '".$_SESSION['userData']['dropship_project_id']."'";
	$result = $conn->query($sql);
	if ($result->num_rows > 0) {
		return true;
	}else{
...
function createSoldier($conn, $asset_name, $name, $title, $rank, $armor, $gear, $level, $ipfs){
	...
	VALUES ('".$asset_name."', '".$name."', '".$title."', '".$rank."', '".$armor."', '".$gear."', '".$level."', '".$ipfs."', '".$_SESSION['userData']['dropship_project_id']."', '".$_SESSION['userData']['dropship_user_id']."')";
...
function updateSoldiers($conn, $asset_names) {
	...
	$sql = "UPDATE soldiers SET user_id = 0 WHERE user_id = '".$_SESSION['userData']['dropship_user_id']."' AND project_id = '".$_SESSION['userData']['dropship_project_id']."' AND rank != 'GRUNT'";
	if ($conn->query($sql) === TRUE) {
	  //echo "New record created successfully";
	} else {
	  //echo "Error: " . $sql . "<br>" . $conn->error;
	...
	$sql = "UPDATE soldiers SET user_id = '".$_SESSION['userData']['dropship_user_id']."' WHERE asset_name IN ('".$asset_names."') AND project_id = '".$_SESSION['userData']['dropship_project_id']."'";
	if ($conn->query($sql) === TRUE) {
	  //echo "New record created successfully";
	} else {
	  //echo "Error: " . $sql . "<br>" . $conn->error;
...
function resetSoldiers($conn){
	$sql = "UPDATE soldiers SET active = 0, deceased = 0 WHERE project_id = '".$_SESSION['userData']['dropship_project_id']."'";
	if ($conn->query($sql) === TRUE) {
	  //echo "Record updated successfully";
	} else {
	  //echo "Error: " . $sql . "<br>" . $conn->error;
...
function checkScore($conn) {
	$sql = "SELECT id, game_id, user_id, score FROM results WHERE user_id='".$_SESSION['userData']['dropship_user_id']."' AND game_id='".$_SESSION['userData']['game_id']."' AND project_id = '".$_SESSION['userData']['dropship_project_id']."'";
	$result = $conn->query($sql);
...
function deleteResult($conn){
	...
	if(isset($result_id)){
		$sql = "DELETE FROM results WHERE id = '".$result_id."' AND project_id = '".$_SESSION['userData']['dropship_project_id']."'";
		if ($conn->query($sql) === TRUE) {
		  //echo "Record deleted successfully";
		} else {
		  //echo "Error: " . $sql . "<br>" . $conn->error;
		}
		$sql = "DELETE FROM results_items WHERE result_id = '".$result_id."' AND project_id = '".$_SESSION['userData']['dropship_project_id']."'";
		if ($conn->query($sql) === TRUE) {
		  //echo "Record deleted successfully";
		} else {
		  //echo "Error: " . $sql . "<br>" . $conn->error;
		}
		$sql = "DELETE FROM results_soldiers WHERE result_id = '".$result_id."' AND project_id = '".$_SESSION['userData']['dropship_project_id']."'";
		if ($conn->query($sql) === TRUE) {
		  //echo "Record deleted successfully";
		} else {
		  //echo "Error: " . $sql . "<br>" . $conn->error;
...
function logScore($conn, $replay) {
	...
	if(isset($_SESSION['userData']['game_id'])) {
		...
		VALUES ('".$_SESSION['userData']['game_id']."', '".$_SESSION['userData']['dropship_user_id']."', '".$_SESSION['userData']['score']."', '".htmlspecialchars($replay)."', '".$_SESSION['userData']['dropship_project_id']."')";
...
function checkActiveInventory($conn) {
	$sql = "SELECT item_id FROM inventory WHERE user_id='".$_SESSION['userData']['dropship_user_id']."' AND active='1' AND project_id = '".$_SESSION['userData']['dropship_project_id']."'";
	$result = $conn->query($sql);
...
function logResultsItems($conn, $item_id) {
	...
	VALUES ('".$result_id."', '".$item_id."', '".$_SESSION['userData']['dropship_project_id']."')";
...
function checkActiveSoldiers($conn) {
	$sql = "SELECT id FROM soldiers WHERE user_id='".$_SESSION['userData']['dropship_user_id']."' AND active='1' AND project_id = '".$_SESSION['userData']['dropship_project_id']."'";
	$result = $conn->query($sql);
...
function logResultsSoldiers($conn, $soldier_id) {
	...
	VALUES ('".$result_id."', '".$soldier_id."', '".$_SESSION['userData']['dropship_project_id']."')";
...
function checkResultID($conn) {
	$sql = "SELECT id FROM results WHERE user_id='".$_SESSION['userData']['dropship_user_id']."' AND game_id='".$_SESSION['userData']['game_id']."' AND project_id = '".$_SESSION['userData']['dropship_project_id']."'";
	$result = $conn->query($sql);
...
function removeInventory($conn) {
	$sql = "UPDATE inventory SET active='0' WHERE user_id='".$_SESSION['userData']['dropship_user_id']."' AND project_id = '".$_SESSION['userData']['dropship_project_id']."'";
	if ($conn->query($sql) === TRUE) {
	  //echo "Record updated successfully";
	} else {
...
function getReplay($conn, $result_id) {
	$sql = "SELECT replay FROM results WHERE id = '".$result_id."' AND project_id = '".$_SESSION['userData']['dropship_project_id']."'";
	$result = $conn->query($sql);
...
function getAverageScore($conn){
	if(isset($_SESSION['userData']['dropship_user_id'])){
		$sql = "SELECT AVG(score) AS score_avg FROM results WHERE results.user_id='".$_SESSION['userData']['dropship_user_id']."' AND project_id = '".$_SESSION['userData']['dropship_project_id']."'";
		$result = $conn->query($sql);
...
function getTopScore($conn){
	if(isset($_SESSION['userData']['dropship_user_id'])){
		$sql = "SELECT MAX(score) AS max_score FROM results WHERE results.user_id='".$_SESSION['userData']['dropship_user_id']."' AND project_id = '".$_SESSION['userData']['dropship_project_id']."'";
		$result = $conn->query($sql);
...
function getScoreCount($conn){
	if(isset($_SESSION['userData']['dropship_user_id'])){
		$sql = "SELECT COUNT(score) AS total_scores FROM results WHERE results.user_id='".$_SESSION['userData']['dropship_user_id']."' AND project_id = '".$_SESSION['userData']['dropship_project_id']."'";
		$result = $conn->query($sql);
...
function loadAchievements($conn, $heavy_armor, $medium_armor, $light_armor, $base_armor, $melee, $demolition, $medkit) {
	...
	if ($result->num_rows > 0) {
	  // output data of each row
	  while($row = $result->fetch_assoc()) {
			$sql = "SELECT item_id FROM inventory WHERE user_id = '".$_SESSION['userData']['dropship_user_id']."' AND item_id = '".$row["id"]."' AND project_id = '".$_SESSION['userData']['dropship_project_id']."'";
	...
	// Scene achievements
	$sql = "SELECT MAX(score) AS max_score FROM results WHERE user_id = '".$_SESSION['userData']['dropship_user_id']."' AND project_id = '".$_SESSION['userData']['dropship_project_id']."'";
	...
	// SCRIP achievements
	$sql = "SELECT SUM(amount) AS total_amount FROM transactions WHERE user_id = '".$_SESSION['userData']['dropship_user_id']."' AND type='credit' AND project_id = '".$_SESSION['userData']['dropship_project_id']."'";
	...
	$sql = "SELECT COUNT(amount) AS amount_count FROM transactions WHERE user_id = '".$_SESSION['userData']['dropship_user_id']."' AND type='credit' AND amount='0' AND project_id = '".$_SESSION['userData']['dropship_project_id']."'";
	...
	if ($result->num_rows > 0) {
	  ...
	  while($row = $result->fetch_assoc()) {
		...
		<table class='inventory-item' width='100%'>
			...
			if(isset($_SESSION['userData']['dropship_user_id'])){
				$sql = "SELECT AVG(results.score) AS score_avg FROM items INNER JOIN results_items ON results_items.item_id = items.id INNER JOIN results ON results.id = results_items.result_id WHERE type='".$type."' AND items.id='".$row['id']."' AND results.user_id='".$_SESSION['userData']['dropship_user_id']."' AND results.project_id = '".$_SESSION['userData']['dropship_project_id']."'";
				...
				$sql = "SELECT COUNT(items.id) AS item_total FROM items INNER JOIN results_items ON results_items.item_id = items.id INNER JOIN results ON results.id = results_items.result_id WHERE type='".$type."' AND items.id='".$row['id']."' AND results.user_id='".$_SESSION['userData']['dropship_user_id']."' AND results.project_id = '".$_SESSION['userData']['dropship_project_id']."'";
...
function buyItem ($conn, $item_id, $drop_box){
	...
	if($balance >= $itemCost){
		...
		if($activeType == "false"){
			...
			$sql = "SELECT user_id, item_id, active FROM inventory WHERE user_id='".$_SESSION['userData']['dropship_user_id']."' AND item_id='".$item_id."' AND project_id = '".$_SESSION['userData']['dropship_project_id']."'";
			$result = $conn->query($sql);
			
			if($itemCost > 0){
				removeBalance($conn, $itemCost, $_SESSION['userData']['dropship_user_id']);
			}
			logDebit($conn, $_SESSION['userData']['dropship_user_id'], $item_id, $itemCost);
			if ($result->num_rows > 0) {
				while($row = $result->fetch_assoc()) {
					...
					if($row["active"] == "0") {
						...
						$sql = "UPDATE inventory SET active='1' WHERE user_id='".$_SESSION['userData']['dropship_user_id']."' AND item_id='".$item_id."' AND project_id = '".$_SESSION['userData']['dropship_project_id']."'";
						if ($conn->query($sql) === TRUE) {
						  //echo "Record updated successfully";
						} else {
						  //echo "Error updating record: " . $conn->error;
			  ...
			  //echo "0 results";
				...
				VALUES ('".$_SESSION['userData']['dropship_user_id']."', '".$item_id."', '".$_SESSION['userData']['dropship_project_id']."', '1')";
...
function checkInventoryItemType($conn, $item_type) {
	// Check if item type is already active in user inventory
	$sql = "SELECT inventory.item_id, inventory.active, items.type FROM inventory INNER JOIN items ON inventory.item_id = items.id WHERE user_id='".$_SESSION['userData']['dropship_user_id']."' AND items.type='".$item_type."' AND inventory.active='1' AND inventory.project_id = '".$_SESSION['userData']['dropship_project_id']."'";
	$result = $conn->query($sql);
...
function removeBalance($conn, $itemCost, $user_id) {
	$sql = "SELECT user_id, balance FROM balances WHERE user_id='".$user_id."' AND project_id = '".$_SESSION['userData']['dropship_project_id']."'";
	$result = $conn->query($sql);
	...
	if ($result->num_rows > 0) {
		while($row = $result->fetch_assoc()) {
			...
			$sql = "UPDATE balances SET balance='".$balance."' WHERE user_id='".$user_id."' AND project_id = '".$_SESSION['userData']['dropship_project_id']."'";
			if ($conn->query($sql) === TRUE) {
			  //echo "Record updated successfully";
			} else {
			  //echo "Error updating record: " . $conn->error;
...
function checkInventory($conn) {
	if(isset($_SESSION['userData']['dropship_user_id'])){
		$sql = "SELECT inventory.user_id, inventory.item_id, inventory.active, items.name, items.type FROM inventory INNER JOIN items ON inventory.item_id = items.id WHERE user_id='".$_SESSION['userData']['dropship_user_id']."' AND inventory.project_id = '".$_SESSION['userData']['dropship_project_id']."' ORDER BY items.type DESC";
		$result = $conn->query($sql);
...
function checkResultsItemsNextLine($conn, $result_id, $clean) {
	$sql = "SELECT items.name FROM results_items INNER JOIN items ON results_items.item_id=items.id WHERE results_items.result_id='".$result_id."' AND results_items.project_id = '".$_SESSION['userData']['dropship_project_id']."' ORDER BY results_items.id DESC";
	$result = $conn->query($sql);
...
function checkPlayerItems($conn) {
	$sql = "SELECT results.id, results.game_id, results.user_id, results.score, users.username FROM results INNER JOIN users ON results.user_id=users.id WHERE users.id ='".$_SESSION['userData']['dropship_user_id']."' AND game_id='".$_SESSION['userData']['game_id']."' AND results.project_id = '".$_SESSION['userData']['dropship_project_id']."' ORDER BY results.score DESC";
	$result = $conn->query($sql);
...
function getResultsSoldiers($conn, $result_id){
	global $prefix;
	$sql = "SELECT name, asset_name, ipfs FROM soldiers INNER JOIN results_soldiers ON soldiers.id=results_soldiers.soldier_id WHERE results_soldiers.result_id='".$result_id."' AND soldiers.project_id = '".$_SESSION['userData']['dropship_project_id']."'";
	$result = $conn->query($sql);
...
function checkLeaderboard($conn, $clean) {
	...
	$sql = "SELECT results.id, results.game_id, results.user_id, results.score, users.username, discord_id, avatar, users.id AS user_id FROM results INNER JOIN users ON results.user_id=users.id WHERE game_id='".$_SESSION['userData']['game_id']."' AND results.project_id = '".$_SESSION['userData']['dropship_project_id']."' ORDER BY results.score DESC";
	$result = $conn->query($sql);
...
function checkATHLeaderboard($conn, $clean) {
	global $discordid_kryptman, $discordid_oculusorbus, $discordid_ohhmeed;
	$sql = "SELECT results.id, results.user_id, results.game_id, users.username, discord_id, avatar, MAX(results.score) as max_score, replay FROM results LEFT JOIN users ON results.user_id = users.id WHERE results.project_id = '".$_SESSION['userData']['dropship_project_id']."' GROUP BY results.user_id ORDER BY max_score DESC";
	$result = $conn->query($sql);
...
function checkMaxScoreResultID($conn, $user_id, $max_score){
	if(isset($user_id)){
		$sql = "SELECT DISTINCT id, user_id, score FROM results WHERE score = '".$max_score."' AND user_id = '".$user_id."' AND project_id = '".$_SESSION['userData']['dropship_project_id']."' ORDER BY id DESC";
		$result = $conn->query($sql);
		while($row = $result->fetch_assoc()) {
			return $row["id"];
		}
...
function checkXP($conn, $user_id) {
	...
	$data['ceiling'] = 100;
	$data['percentage'] = 0;
	$sql = "SELECT SUM(results.score) as xp FROM results INNER JOIN users ON results.user_id=users.id WHERE users.id='".$user_id."' AND project_id = '".$_SESSION['userData']['dropship_project_id']."'";
	$result = $conn->query($sql);
...
function checkXPLeaderboard($conn, $clean) {
	$sql = "SELECT results.game_id, results.user_id, SUM(results.score) as xp, users.username, discord_id, avatar FROM results INNER JOIN users ON results.user_id=users.id WHERE results.project_id = '".$_SESSION['userData']['dropship_project_id']."' GROUP BY results.user_id ORDER BY xp DESC";
	$result = $conn->query($sql);
...
function deactivateGame($conn) {
	...
	resetSoldiers($conn);
	$sql = "UPDATE games SET active='0' WHERE id='".$_SESSION['userData']['game_id']."' AND project_id = '".$_SESSION['userData']['dropship_project_id']."'";
...
function logBalances($conn) {
	$sql = "SELECT results.id, results.game_id, results.user_id, results.score, users.username, users.discord_id, games.prizes FROM results INNER JOIN users ON results.user_id=users.id INNER JOIN games ON results.game_id = games.id WHERE game_id='".$_SESSION['userData']['game_id']."' AND results.project_id = '".$_SESSION['userData']['dropship_project_id']."' ORDER BY results.score DESC";
	$result = $conn->query($sql);
	  ...
	  // output data of each row
		...
		while($row = $result->fetch_assoc()) {
			...
			if($counter > $row["prizes"]){
				...
				if($_SESSION['userData']['dropship_project_id'] == 1 || $_SESSION['userData']['dropship_project_id'] == 4){
					// set post fields
					$post = [
...
function logBalance($conn, $user_id, $score) {
	...
	if ($result->num_rows > 0) {
		while($row = $result->fetch_assoc()) {
			...
			// If user does exist, update record for calculated balance
			$sql = "UPDATE balances SET balance='".$score."' WHERE user_id='".$user_id."' AND project_id = '".$_SESSION['userData']['dropship_project_id']."'";
			if ($conn->query($sql) === TRUE) {
			  //echo "Record updated successfully";
			} else {
			  //echo "Error updating record: " . $conn->error;
	  ...
	  //echo "0 results";
		...
		VALUES ('".$user_id."', '".$score."', '".$_SESSION['userData']['dropship_project_id']."')";
...
function logCredit($conn, $user_id, $result_id, $amount) {
	...
	VALUES ('credit', '".$user_id."', '".$result_id."', '".$amount."', '".$_SESSION['userData']['dropship_project_id']."')";
...
function logDebit($conn, $user_id, $item_id, $amount) {
	...
	VALUES ('debit', '".$user_id."', '".$item_id."', '".$amount."', '".$_SESSION['userData']['dropship_project_id']."')";
...
function logBattleCredit($conn, $user_id, $battle_id, $amount) {
	...
	VALUES ('credit', '".$user_id."', '".$battle_id."', '".$amount."', '".$_SESSION['userData']['dropship_project_id']."')";
...
function logBattleDebit($conn, $user_id, $battle_id, $amount) {
	$sql = "INSERT INTO transactions (type, user_id, battle_id, amount, project_id)
	VALUES ('debit', '".$user_id."', '".$battle_id."', '".$amount."', '".$_SESSION['userData']['dropship_project_id']."')";
...
function transactionHistory($conn) {
	if(isset($_SESSION['userData']['dropship_user_id'])){
		$sql = "SELECT transactions.type, amount, items.name, transactions.date_created, results.game_id AS game_id, results.score AS score, battle_id, user_score, opponent_score, battles.user_id AS creator_id FROM transactions LEFT JOIN results ON transactions.result_id=results.id LEFT JOIN items ON transactions.item_id = items.id LEFT JOIN battles ON transactions.battle_id = battles.id WHERE transactions.user_id='".$_SESSION['userData']['dropship_user_id']."' AND transactions.project_id = '".$_SESSION['userData']['dropship_project_id']."' ORDER BY date_created DESC";
		$result = $conn->query($sql);
		...
		while($row = $result->fetch_assoc()) {
			...
			if ($row["type"] == "credit"){
				...
				if($row["battle_id"] != 0){
					echo $win;
					echo "</td><td>";
					if($row["creator_id"] == $_SESSION['userData']['dropship_user_id']){
...
function checkBalance($conn) {
	if(isset($_SESSION['userData']['dropship_user_id'])){
		$sql = "SELECT user_id, balance FROM balances WHERE user_id='".$_SESSION['userData']['dropship_user_id']."' AND project_id = '".$_SESSION['userData']['dropship_project_id']."'";
		$result = $conn->query($sql);
...
function createGame($conn, $name, $prizes) {
	...
	VALUES ('".$name."','".$prizes."', '1', '".$_SESSION['userData']['dropship_project_id']."')";
